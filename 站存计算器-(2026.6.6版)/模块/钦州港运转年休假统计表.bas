...
'                            If 姓名 = "詹鋆亮" Then Stop '调试
                ...
                截止时间 = Format(DateSerial(Year(Now), "01", "01") - 1, "yyyy-mm-dd")
                If 结束时间 < 截止时间 Then
                    Exit For
                End If
            Next
'                On Error GoTo 0
        ...
        For y = 2 To 16 Step 4
            ...
            For Each rn In Qy
                '//将数据写入对应的单元格
                rn.Offset(0, 1).value = d(rn.value)(1) '//职名
                If rn.Offset(0, 2).value = "" Then rn.Offset(0, 2).value = d(rn.value)(3) '//年休时间
                If rn.Offset(0, 3).value = "" Then rn.Offset(0, 3).value = d(rn.value)(4) '//假期天数
